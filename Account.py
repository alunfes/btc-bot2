...
class Account:
    ...
    def move_to_next(self, ind, i):
        ...
        self.total_pl_log[i] = self.realized_pl + self.unrealized_pl
    ...
    def entry_order(self, side, price, size, ind, i):
        if self.cancel_all_orders_flg == False:
            self.unexe_position.append(side)
            self.unexe_price.append(price)
            self.unexe_lot.append(size)
            self.unexe_time.append(MarketData2.ohlc_bot.close[ind])
            self.unexe_i.append(i)
            self.unexe_cancel.append(False)
            self.__add_action_log("Entry Order for " + side + " @" + str(price) + " x " + str(size),i)

    def exit_all_orders(self, ind, i):
        side = 'buy' if self.holding_side == 'buy' else 'sell'
    ...
    '''
    def entry_price_tracing_order(self, side, target_lot, ind, i):
        self.price_tracing_order_dt = MarketData2.ohlc_bot.dt[ind]
        self.price_tracing_order_position = side
        self.price_tracing_order_i = i
        self.price_tracing_order_flg = True
        self.price_tracing_order_target_lot = target_lot
        self.cancel_all_orders(ind, i)
        self.__add_action_log("Started Price Tracing Order for " + side + " x" + str(target_lot),i)

    def __check_exec_price_tracing_order(self, ind, i):
        if self.price_tracing_order_i < i:
            price = round((MarketData2.ohlc_bot.low[ind] + MarketData2.ohlc_bot.high[ind]) * 0.5)
            self.__entry_order(self.price_tracing_order_side, price, self.price_tracing_order_target_size, ind, i-1)
    '''
    ...
    def __check_cancel(self, ind, i):
        if self.cancel_all_orders_flg:
            ...
            cancelled_index = ''
            for j in range(len(self.unexe_cancel)):
